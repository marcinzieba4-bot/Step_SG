...
Sizing: each (day, expiry) tranche sells notional = NAV * leverage / 9.
...
class Params:
    ...
    include_cash: bool = True        # collateral earns the T-bill rate
...
def run_backtest(df: pd.DataFrame, p: Params | None = None, start: str | None = None) -> Result:
    ...
    for i in range(1, len(cal_hist)):
        ...
        # 1) cash accrual on collateral (previous rate)
        interest = cash * rf[i - 1] * dt if p.include_cash else 0.0
        cash += interest

        # 2) new sales at today's TWAP, sized off yesterday's NAV
        nav_prev = nav
        ...
        for exp_pos, bd in expiries_for(i, cal):
            ...
            if fill <= 0:
                continue
            notional = nav_prev * p.leverage / p.tranche_divisor
            ...
            pos = dict(
                ...
                price_mid=px, fill=fill, units=units, notional=notional,
            )
        ...
        liab = delta = gamma = vega = gross_notional = 0.0
        for pos in positions:
